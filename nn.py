...
class NeuralNet:
    ...
    def learn(self):
        synapses = []
        synapses.append(2 * np.random.rand(X.shape[1], self.hidden[0]) - 1)
        ...
        for i in range(0, self.iterations):
            ...
            layers = self.forward(X, synapses)

            deltas = []
            for i in range(0, len(synapses)):
                if i == 0:
                    self.error = (Y - layers[reverse - i])
            ...
            for i in range(0, len(synapses)):
                if i == len(synapses) - 1:
                    synapses[reverse - i] = synapses[reverse - i] + X.T.dot(deltas[i])
                else:
